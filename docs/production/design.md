...
P02 Task 5 implements `when.resolve_time_context(envelope, now, resumed) -> TimeContext` with the original timezone and capture reference. Since current Notes envelopes cannot prove original capture freshness, observed-only relative dates require an explicit date even on first observation. Date-only reminders carry no explicit timed alarm. Calendar creation requires a user-supplied end or duration; no one-hour default is applied. ISO offsets survive native transport, and interval comparisons use actual instants through DST changes.

Calendar/list/reminder resolution returns candidate sets. Selected stable IDs persist in `Action.target_id`; `Action.timezone` carries the capture zone. Unknown or duplicate names ask. Until P03 adds conversation references, a current request can specify a separate `calendar-id <id>`, `list-id <id>`, or `reminder-id <id>` line. Conflicts ask the user to restate the event with `confirm conflict <code>` on a separate line; the code binds the full event, destination ID and current conflicts. Model output does not supply these authorizations. Known pre-effect refusals deliver a clarification through Notes; uncertain outcomes retain review status. Existing unbound PREPARED intents require review, while APPLIED effects can still repair receipts.

Indexed queries validate at most eight selected note IDs. Changed notes are reread under policy and bounded to 1,400 characters, with a second metadata check; this can require up to three Notes calls per changed note. Unavailable or omitted context gets a deterministic incomplete-context disclosure. Ranking still uses existing embeddings; no query-time rebuild is required. Calendar summaries disclose their bounds and truncation; unavailable access is not an empty schedule. These checks do not make external Apple edits transactional.
