...
def _library_is_never_the_developers(monkeypatch, tmp_path, _policy_is_disposable, _notes_is_never_the_real_one):
    ...
    _notes_is_never_the_real_one.folders[0][1].extend(['Recipes', 'Private', 'Old'])
...
def test_a_note_hidden_only_by_the_year_cutoff_is_also_never_asked(_notes_is_never_the_real_one, monkeypatch):
    ...
    library.save(library.Library(start_from=library.parse_start("2026"), allow_new_notes=True))
    ...
    monkeypatch.setattr(attachments.notes, 'get_note', lambda nid: attachments.notes.Note(nid, 'Old', 'Notes', recent))
...
def test_a_file_comes_out_of_notes_onto_disk(_notes_is_never_the_real_one, cache):
    ...
    assert attachments.fetch(att) == b"\x89PNG pretend"
    assert not cache.exists()
    assert b'pretend' not in cache.with_suffix('.enc').read_bytes()
...
def test_a_folder_lists_only_readable_notes(_notes_is_never_the_real_one):
    ...
    assert app.calls.count("in_folder") == 0
...
def test_a_folder_that_moved_underneath_us_reports_nothing(
    ...
    assert 'Notes/Parking Garages' in attachments.in_folder("Notes")


def test_cached_content_is_purged_when_source_is_revoked(_notes_is_never_the_real_one, cache):
    app = _notes_is_never_the_real_one
    app.attachments['Notes/Recipes'] = [('photo.png', 'att/purge')]
    app.files['att/purge'] = b'private-original'
    att = attachments.on_note('Notes/Recipes')[0]
    attachments.fetch(att)
    attachments._put(att, words='private-derived-words')
    assert b'private' not in cache.with_suffix('.enc').read_bytes()
    library.save(library.Library(ignore={'Notes/Recipes'}))
    attachments.purge()
    assert attachments._load() == {}
    with pytest.raises(attachments.NotAllowed):
        attachments.known_text(att)


def test_deleted_source_cannot_reuse_encrypted_original(_notes_is_never_the_real_one, cache):
    app = _notes_is_never_the_real_one
    app.attachments['Notes/Recipes'] = [('photo.png', 'att/deleted')]
    app.files['att/deleted'] = b'private-original'
    att = attachments.on_note('Notes/Recipes')[0]
    attachments.fetch(att)
    app.folders[0][1].remove('Recipes')
    with pytest.raises(attachments.NotAllowed):
        attachments.fetch(att)
    attachments.purge(set())
    assert not attachments._load()


def test_temporary_audio_is_removed_after_recognizer_failure(_notes_is_never_the_real_one, cache, monkeypatch):
    app = _notes_is_never_the_real_one
    app.attachments['Notes/Recipes'] = [('memo.m4a', 'att/temp')]
    app.files['att/temp'] = b'private-audio'
    seen = []
    def fail(path):
        seen.append(path)
        assert path.read_bytes() == b'private-audio'
        assert path.parent.stat().st_mode & 0o777 == 0o700
        raise RuntimeError('synthetic')
    monkeypatch.setattr(attachments, '_listen', fail)
    with pytest.raises(RuntimeError):
        attachments.transcribe(attachments.on_note('Notes/Recipes')[0])
    assert seen and not seen[0].exists() and not seen[0].parent.exists()


def test_missing_key_never_extracts_to_plaintext(_notes_is_never_the_real_one, cache, monkeypatch):
    from notron import credentials
    app = _notes_is_never_the_real_one
    att = attachments.Attachment('att/no-key', 'memo.m4a', 'audio', 'Notes/Recipes')
